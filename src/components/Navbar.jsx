			...
			<div className="navbar__left flex items-center justify-center gap-3">
				<div className="w-40 bg-blend-screen items-center justify-center">
					<Link to={'/'}>
						<div className="w-40">
							{' '}
							<img className="w-40 h-full object-cover" src="https://i.ibb.co/CwFssJC/logo.png" alt="" />
						</div>
					</Link>
				</div>
				<div className="hidden md:flex items-center justify-center gap-2 font-[500]">
					<NavLink to={'/'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
						<li className=" flex justify-center items-center gap-1 cursor-pointer">
							{' '}
							<i className="ri-home-line"></i> Home
						</li>
					</NavLink>
					<NavLink to={'/category'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
						<li className=" flex justify-center items-center gap-1 cursor-pointer">
							{' '}
			...
			<div className="navbar__right flex items-center text-xl md:text-base gap-2 md:gap-5 font-[500] capitalize relative ">
				<button className="hover:bg-primary group/item w-32 h-10 rounded-lg hover:text-white transition-all group hidden lg:block">
					<li className=" flex justify-center items-center gap-1 cursor-pointer capitalize">
						<i className="ri-user-line"></i> account <i className="ri-arrow-down-s-line group-hover:rotate-180 transition-all"></i>
					</li>
					...
					<ul className=" absolute top-[43px] w-72 left-[-30px] scale-0  group-hover/item:scale-100  origin-top-left duration-100 bg-white text-[#383838] rounded-lg text-left  shadow-inner overflow-hidden">
						{/* my profile */}
						<NavLink to={'/profile'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<i className="ri-user-line"></i> My profile
							</li>
						</NavLink>
						{/* mebership */}
						<NavLink to={'/mebership'} className={`  ({ isActive }) => isActive ? 'text-primary' : 'text-[#383838] '`}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<AddTaskIcon fontSize="10px" /> mebership exclusive offer
							</li>
						</NavLink>
						{/* orders  */}
						<NavLink to={'/orders'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<i className="ri-instance-line"></i> orders
							</li>
						</NavLink>
						{/* wishlist  */}
						<NavLink to={'/wishlist'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<i className="ri-heart-3-line"></i> wishlist
							</li>
						</NavLink>
						{/* your rewords  */}
						<NavLink to={'/rewords'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<i className="ri-medal-line"></i> your rewords
							</li>
						</NavLink>
						{/* your earing or money */}
						<NavLink to={'/earning'} className={`  ({ isActive }) => isActive ? 'text-primary' : 'text-[#383838]'`}>
							<li className="my_profile px-4 pt-3 pb-3 hover:bg-primary hover:text-white">
								<i className="ri-money-dollar-circle-line"></i> your earing or money
							</li>
						</NavLink>
					</ul>
				</button>

				<NavLink to={'/cart'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
					<li className=" flex justify-center items-center gap-1 cursor-pointer">
						{' '}
				...
				<div className={`md:hidden `}>
					...
					<div className={`fixed w-72 h-screen bg-white top-0 overflow-auto ${menu ? 'right-0' : 'right-[-300px]'} duration-150`}>
						...
						<ul className="">
							...
							<NavLink to={'/Categories'} className={({ isActive }) => (isActive ? 'text-primary' : 'text-[#383838]')}>
								<li className=" hover:bg-primary hover:text-white flex gap-2 pt-3 pb-3 px-3 cursor-pointer">
							...
							<li className="relative flex flex-col  gap-2 pt-3 pb-3 px-3 cursor-pointer">
								...
								>
									...
									<i className={`ri-arrow-down-s-line absolute right-3 duration-200 ${accountDropdown ? 'rotate-180' : ''}`}></i>
								</div>

								{/* account dropdown */}
								<ul
									className={` ${
										accountDropdown ? 'block' : 'hidden'
